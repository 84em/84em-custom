...
class GoogleReviewsBlock {
	...
	public function init(): void {
		wp_register_script(
			'google-reviews-block-editor',
			plugin_dir_url( __FILE__ ) . 'google-reviews-block/block.js',
			[ 'wp-blocks', 'wp-element', 'wp-editor', 'wp-components', 'wp-i18n' ],
			filemtime( plugin_dir_path( __FILE__ ) . 'google-reviews-block/block.js' )
		);

		wp_register_style(
			'google-reviews-block-editor',
			plugin_dir_url( __FILE__ ) . 'google-reviews-block/editor.css',
			[ 'wp-edit-blocks' ],
			filemtime( plugin_dir_path( __FILE__ ) . 'google-reviews-block/editor.css' )
		);

		wp_register_style(
			'google-reviews-block-style',
			plugin_dir_url( __FILE__ ) . 'google-reviews-block/style.css',
			[],
			filemtime( plugin_dir_path( __FILE__ ) . 'google-reviews-block/style.css' )
		);
	...
	private function format_review_time( int $timestamp ): string {
		...
		$days_ago    = floor( ( time() - $timestamp ) / ( 24 * 60 * 60 ) );
